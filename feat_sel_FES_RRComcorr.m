function [data_mod,index_rem,Hrlc]=feat_sel_FES_RRComcorr(data, measure,p)
% Function call:
...
% De Luca's entropy measure
    ...
    H1=sum(-sim.*log(sim)-(1-sim).*log(1-sim));
    
elseif measure=='park'
    H1=sum(sin(pi/2*sim)+sin(pi/2*(1-sim))-1);  
end

% Simply using Pearson's correlation:
tmp=corr(data);
tmp2=tmp-diag(ones(1,t+1));
H3=1-abs(tmp2(1:t,t+1))';

%True complementarity information addition:
for i=1:t
    tmp=datalearn_s(:,i);
    for j=1:t
        Inte(:,j)=intersec(tmp,datalearn_s(:,j));
    end
    tmp=corr([Inte,data(:,t+1)],'type','Kendall');
    tmp2=tmp-diag(ones(1,t+1));
    Dnew(i)=max(abs(tmp2(1:t,t+1)));
end

H4=1-Dnew; % for correlation

H1=H1./max(H1);

Hrlc = H1+H4; % relevance + complementarity

% find maximum feature
[i,index_rem]=max(Hrlc);
